...
if api_key:
    ...
    # ================= 🚨 修复：使用最新模型名称 =================
    # gemini-pro 和 gemini-pro-vision 已弃用，改用 gemini-1.5-flash
    # gemini-1.5-flash 同时支持文本和图片
...
if selected_user and selected_user != "➕ 新建身体档案...":
    ...
    # 🚨 修复：安全获取calories，防止KeyError
    consumed = sum([daily[k].get('calories', 0) for k in ['breakfast', 'lunch', 'dinner'] if daily[k] and isinstance(daily[k], dict)])
    burned = daily['exercise'].get('burned_calories', 0) if daily['exercise'] and isinstance(daily['exercise'], dict) else 0
    ...
    with col_left:
        with st.container():
            ...
            with tab1:
                ...
                uploaded_img = st.file_uploader("上传美食照片", type=["jpg", "png", "webp"], key=f"img_{meal_key}")
                meal_input = st.text_area("补充文字说明 (例如：油很大，半碗饭)", height=100, key=f"text_{meal_key}")
                
                # 🚨 修复：添加唯一key防止按钮冲突
                if st.button("✨ 开启 AI 深度分析", key=f"btn_{meal_key}"):
                    if not meal_input and not uploaded_img: 
                        ...
                        with st.spinner("AI 营养师正在分析..."):
                            ...
                            try:
                                # 🚨 修复：使用统一的 model，gemini-1.5-flash 同时支持图文
                                contents = [prompt]
                                if uploaded_img:
            ...
            with tab2:
                exercise_input = st.text_area("今天做了什么运动？", height=100, key="exercise_input_tab2")
                if st.button("🔥 计算消耗", key="btn_exercise"):
                    with st.spinner("计算中..."):
    ...
    with col_right:
        with st.container():
            ...
            for m_key, m_name, icon in [("breakfast", "早餐", "🥞"), ("lunch", "午餐", "🍱"), ("dinner", "晚餐", "🥗")]:
                if daily[m_key] and isinstance(daily[m_key], dict):
                    with st.expander(f"{icon} {m_name}账单 (+{daily[m_key].get('calories', 0)} kcal)", expanded=True):
                        st.write(f"**内容**: {daily[m_key].get('text', '')}\n\n**点评**: {daily[m_key].get('analysis', '')}")
            
            if daily['exercise'] and isinstance(daily['exercise'], dict):
                with st.expander(f"🏃 运动流水 (-{daily['exercise'].get('burned_calories', 0)} kcal)", expanded=True):
                    st.write(f"**内容**: {daily['exercise'].get('text', '')}\n\n**点评**: {daily['exercise'].get('analysis', '')}")
            
            st.markdown("---")
            st.markdown("### 🥗 每日营养闭环分析")
            if st.button("🧬 召唤 AI 分析今日整体营养缺口", key="btn_nutrition"):
                meals_dict = {k: daily[k] for k in ['breakfast','lunch','dinner'] if daily[k] and isinstance(daily[k], dict)}
...
if st.button("📈 生成当月 AI 深度诊断报告", key="btn_monthly"):
    ...
    if len(all_data) < 3:
        ...
        with st.spinner("AI 正在深度挖掘您过去记录的所有饮食与运动数据，出具千字级未来规划报告..."):
            try:
                ...
                u_data = users[selected_user]
                prompt = f"""你是一位顶级的身材管理专家。这是用户({u_data['gender']}, {u_data['weight']}kg, 减脂期)当月的所有饮食和运动打卡日志：
{history_str}
